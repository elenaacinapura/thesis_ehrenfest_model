...
\begin{ndef} [Discrete Markov chain] \label{def:markov-chain}
    A sequence of states $\{X_0, X_1, X_2, \dots, X_t\}$ is said a \emph{discrete Markov chain} if the following property holds $\forall t \in \mathbb N$ and $\forall x_0, \dots, x_t \in \Space$:
    \begin{equation}
...
\begin{ndef} [Stationary distribution]
    ...
    \begin{equation}
        P \staz = \staz
    \end{equation}
    Hence, the stationary distribution is the \text{eigenvalue} of the stochastic matrix with unitary eigenvalue. If the system happens to be found in the stationary distribution, it will remain in that same state forever.
\end{ndef}
...
As one might expect, the knowledge of the limiting behaviour of the Markov chain can help answer these questions, and this section is devoted to that. As before, we will start with some simple definition, and then give the relevant theorems.
...
\begin{ndef} [Number of visits]
    ...
    \begin{equation}
        n_i (X_0, T) \equiv \sum_{t = 0}^T \mathbb{1}_i(X_t) \quad , \quad \text{given a certain }X_0
    \end{equation}
    where
    \begin{equation}
        \mathbb{1}_i(X_t) =
...
\begin{ndef} [Mean visit frequency and mean recurrence time] \label{def:nu-tau}
    ...
    \end{equation}
\end{ndef}
Notice that the definition of the mean recurrence time can be interpreted as the \emph{average} time one has to wait before the state can return to a given.
...
\begin{proof}
   Suppose that the system starts at $X_0 = x_j$; then 
    ...
    \end{equation} 
    Now, if the chain has a limiting distribution $\p$, then $\p$ is also a stationary distribution for the matrix $P$ after theorem \ref{th:limit-stat}. This implies that, $\forall t$
